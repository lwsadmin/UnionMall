...
{
    public class AliPayNoteAppService : ApplicationService, IAliPayNoteAppService
    {
        private readonly ISqlExecuter _sqlExecuter;
        public readonly IAbpSession _AbpSession;

        private readonly IRepository<Entity.AliPayNote, long> _Repository;
        public AliPayNoteAppService(ISqlExecuter sqlExecuter, IRepository<Entity.AliPayNote, long> Repository, IAbpSession AbpSession)
        {
...
CONVERT(nvarchar,n.CreationTime,120) CreationTime,n.memo  from dbo.TAlipaynote n 
...
cast(sum( amount) as float) TotalValue
 from dbo.TAlipaynote n left join dbo.TChainStore c on n.ChainStoreId=c.Id where DATEADD(YEAR,1,n.CreationTime)>=GETDATE() {  where = where.Replace("*.BusinessId", "c.BusinessId").Replace("*", "n")} 
...
cast(sum( amount) as float) TotalValue from TAliPayNote n
